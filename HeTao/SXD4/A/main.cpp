...
#include <bits/stdc++.h>
using namespace std;
static const int MAXN = 2e5 + 1000;
struct FHQ {
...
int n, a[N], idx;
int main() {
    // freopen("..\\HeTao\\SXD4\\A\\inversion4.in", "r", stdin);
    // freopen("..\\HeTao\\SXD4\\A\\inversion.out", "w", stdout);
    ...
    long long ans = 0;
    for (int i = 0; i < n; i++) {
        auto it = tr.lower_bound(make_pair(a[i], 0));
        if (it == tr.end()) {
            if (i - 1 >= 0) fhq.prefix_add(i - 1, 1);
            fhq.insert(i, a[i]);
            tr.insert(make_pair(a[i], ++idx));
            continue;
        }
        int rk = tr.order_of_key(*it), c1 = i - rk;
        auto tmp = fhq.suffix_min(rk);
        int c2 = tmp.first - a[i];
        if (c1 <= c2) {
            if (rk - 1 >= 0) fhq.prefix_add(rk - 1, 1);
            fhq.insert(rk, c1 + a[i]);
            tr.insert(make_pair(a[i], ++idx));
        } else {
            int mv = i - tmp.second - 1, trv = tmp.first - mv;
            fhq.prefix_add(tmp.second, 1);
            fhq.insert(tmp.second + 1, tmp.first);
            tr.insert(make_pair(trv, ++idx));
        }
        ans += min(c1, c2);
    }
    cout << ans << '\n';
